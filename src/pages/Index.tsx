...
import { useFeaturedTours, useReviews, useOffers } from "@/hooks/useContent";
...
const Index = () => {
  const { t } = useI18n();
  const { data: featured = [] } = useFeaturedTours();
  ...
  return (
    <div>
        ...
        <div className="relative z-10 container-tour text-center text-primary-foreground">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href={getWhatsAppLink()} target="_blank" rel="noopener noreferrer"
              ...
              <MessageCircle className="h-5 w-5" /> {t("book_now")}
            </a>
            <a href={getEbookWhatsAppLink()} target="_blank" rel="noopener noreferrer"
              ...
              <BookOpen className="h-5 w-5" /> {t("get_ebook")}
            </a>
          </div>
        </div>
      </section>
      ...
      <section className="section-padding">
        <div className="container-tour">
          <h2 className="font-display text-3xl md:text-4xl font-bold text-center text-foreground mb-4">{t("popular_tours")}</h2>
          <p className="text-center text-muted-foreground mb-12 max-w-2xl mx-auto">Our most loved experiences in Marsa Alam — hand-picked for unforgettable memories.</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {featured.map(tour => <TourCard key={tour.id} tour={tour} />)}
          </div>
          <div className="text-center mt-10">
            <Link to="/tours" className="inline-flex items-center gap-2 px-8 py-3 rounded-xl bg-primary text-primary-foreground font-semibold hover:bg-primary/90 transition-colors">
      ...
      <section className="section-padding bg-secondary/50">
        <div className="container-tour">
          <h2 className="font-display text-3xl md:text-4xl font-bold text-center text-foreground mb-12">{t("reviews_title")}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {reviewsData.slice(0, 6).map((r, i) => <ReviewCard key={r.id || i} {...r} />)}
          </div>
        </div>
      </section>
      ...
      <section className="section-padding">
        <div className="container-tour">
          <h2 className="font-display text-3xl md:text-4xl font-bold text-center text-foreground mb-12">{t("offers_title")}</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {offersData.map((o) => (
              <div key={o.id} className="relative bg-card rounded-xl p-6 shadow-card border border-border overflow-hidden group hover:shadow-elevated transition-shadow">
                <div className="absolute top-0 right-0 bg-accent text-accent-foreground px-4 py-2 rounded-bl-xl font-bold text-sm">{o.discount}</div>
                <Gift className="h-10 w-10 text-primary mb-4" />
                <h3 className="font-display text-xl font-bold text-foreground mb-2">{o.title}</h3>
                <p className="text-muted-foreground text-sm mb-3">{o.description}</p>
      ...
      <section className="section-padding gradient-ocean text-primary-foreground">
        <div className="container-tour text-center">
          ...
          <a href={getEbookWhatsAppLink()} target="_blank" rel="noopener noreferrer"
            ...
            <MessageCircle className="h-5 w-5" /> {t("send_guide")}
          </a>
        </div>
      </section>
    </div>
