...
import { useTourBySlug, useTourFaqs, useTours } from "@/hooks/useContent";
...
const TourDetail = () => {
  ...
  const { t } = useI18n();
  const { data: tour, isLoading } = useTourBySlug(slug || "");
  const { data: faqs = [] } = useTourFaqs(tour?.id);
  const { data: allTours = [] } = useTours();

  if (isLoading) {
    return <div className="pt-16 section-padding text-center"><p>Loading...</p></div>;
  }
  ...
  const related = allTours.filter(t => (tour.related_slugs || []).includes(t.slug));
  ...
  return (
    <div className="pt-16">
      ...
      <div className="relative h-[50vh] min-h-[400px]">
        <img src={image} alt={tour.alt_text || tour.title} className="w-full h-full object-cover" />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8">
          <div className="container-tour">
            ...
            <p className="text-primary-foreground/80 text-lg max-w-2xl">{tour.short_desc}</p>
          </div>
        </div>
      </div>
      ...
      <div className="container-tour section-padding">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-10">
            <div><p className="text-foreground/80 text-lg leading-relaxed">{tour.description}</p></div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {(tour.highlights || []).map((h, i) => (
                  <div key={i} className="flex items-center gap-2 text-foreground/80">
                    <div className="h-2 w-2 rounded-full bg-primary" />{h}
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-3">
                {(tour.program || []).map((step, i) => (
                  <div key={i} className="flex items-start gap-3 bg-secondary/50 rounded-lg p-4">
                    <div className="h-6 w-6 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-xs font-bold shrink-0 mt-0.5">{i + 1}</div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <img src={image} alt={`${tour.title} — photo 1`} className="rounded-xl w-full h-48 object-cover" loading="lazy" />
                <img src={image} alt={`${tour.title} — photo 2`} className="rounded-xl w-full h-48 object-cover" loading="lazy" />
              </div>
            </div>

            {faqs.length > 0 && (
              <FAQSection faqs={faqs.map(f => ({ question: f.question, answer: f.answer }))} tourTitle={tour.title} />
            )}
          </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 bg-card rounded-xl shadow-elevated border border-border p-6 space-y-6">
              <div>
                ...
                {!tour.price.startsWith("Price") && <p className="text-sm text-muted-foreground">{t("per_person")}</p>}
              </div>
              {tour.price_note && <p className="text-sm text-muted-foreground bg-secondary/50 rounded-lg p-3">{tour.price_note}</p>}
              <a href={getWhatsAppLink(tour.title)} target="_blank" rel="noopener noreferrer"
                ...
                <MessageCircle className="h-5 w-5" /> {t("book_whatsapp")}
              </a>
              <p className="text-xs text-center text-muted-foreground">Fast response — typically within 30 minutes!</p>
